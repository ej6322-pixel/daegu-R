...
# ── API 키 설정 (Streamlit Cloud Secrets 우선, 환경변수 fallback) ──
...
def extract_events(uploaded_files, store_name):
...
if st.button("🤖 AI 분석 시작", type="primary", use_container_width=True):
    if not lotte_files and not hyundai_files:
        ...
        st.error("ANTHROPIC_API_KEY가 설정되지 않았습니다. Streamlit Cloud > Secrets에 추가해주세요.")
    else:
        with st.spinner("이미지 분석 중... (20~40초 소요)"):
            try:
                lotte_ev = extract_events(lotte_files[:10], "롯데백화점 대구점")
                hyundai_ev = extract_events(hyundai_files[:10], "더현대 대구")
